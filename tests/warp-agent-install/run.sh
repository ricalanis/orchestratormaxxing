...
case "$*" in
  ...
  'plugin list --json')
    printf '{"installed":[{"pluginId":"claudemaxxing@personal","installed":true,"enabled":true,"version":"%s"}]}\n' "$STUB_VERSION" ;;
  *) printf '{}\n' ;;
esac
SH
cat > "$STUBS/tmux" <<'SH'
...
case "$1" in
  ...
  print)
    ...
    case "${2:-}" in
      gui/*/com.claudemaxxing.loop|gui/*/com.claudemaxxing.cogload-nightly|gui/*/com.claudemaxxing.cogload-catchup)
        _label="${2##*/}"
...
STUB_VERSION="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["version"])' \
  "$ROOT/plugins/claudemaxxing/.codex-plugin/plugin.json")"

run_install() {
  HOME="$HOME_DIR" CODEX_HOME="$HOME_DIR/.codex" \
    CLAUDEMAXXING_SKIP_EXTERNAL_SKILLS=1 \
...
if [ "$(uname -s)" = Darwin ]; then
  for label in com.claudemaxxing.loop \
               com.claudemaxxing.cogload-nightly \
               com.claudemaxxing.cogload-catchup; do
  ...
  STUB_LAUNCHCTL_HOME="/Users/someone-else" HOME="$HOME_DIR" CODEX_HOME="$HOME_DIR/.codex" \
    PATH="$STUBS:$PATH" CLAUDEMAXXING_SKIP_EXTERNAL_SKILLS=1 bash "$ROOT/install.sh" >/dev/null 2>&1 || true
...
backup = home / ".codex/hooks.json.pre-claudemaxxing-plugin"
...
plugin_hooks = json.loads((pathlib.Path(sys.argv[2]) / "plugins/claudemaxxing/hooks/hooks.json").read_text())
plugin_watchers = [
    hook.get("command", "")
...
assert tmux.count("# >>> claudemaxxing:tmux >>>") == 1
...
for rc in (home / ".bashrc", home / ".zshrc"):
    text = rc.read_text()
    assert text.count("# >>> claudemaxxing:c-command >>>") == 1
...
for name in ("warp-recovery.sh", "claude-c.sh", "codex-g.sh", "opencode-o.sh"):
    deployed = home / ".config/claudemaxxing" / name
...
assert "CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1" in (
    home / ".config/claudemaxxing/claude-c.sh").read_text()
installed_c = (home / ".config/claudemaxxing/claude-c.sh").read_text()
installed_g = (home / ".config/claudemaxxing/codex-g.sh").read_text()
...
plugin = home / ".config/opencode/plugins/claudemaxxing-notify.js"
