...
3. **Pick the lane** from the playbook's table, cheapest adequate first — unless
   the task text begins with the escalation sigil ****, in which case strip the
   ...
   | OpenCode worker | `o delegate <run-id> --agent kimi-coder\|glm-coder\|minimax-coder --run-dir .results/delegation/<run-id> --json` | Workspace reads/edits, commands/tests, persistent artifacts, repairs via `o send` |
   ...
   For workspace work, run `o delegate <run-id> --agent <agent> --run-dir
   .results/delegation/<run-id> --json`; reuse the returned session with `o send`
   for repairs and inspect only bounded transport state with `o output --json`.
   The `occ` wrapper is an internal one-shot transport behind `o`, not a host-facing
   delegation lane. Transport success never accepts the worker artifact.
...
4. **Dispatch from the persisted brief + contract**, then **verify by contract
   ...
   A successful `o output` capture still must pass the original caller-authored
