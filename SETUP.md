...
`CLAUDEMAXXING_SKIP_GSTACK=1 ./bootstrap.sh` skips the gstack clone/setup (the default installs
...
| Codex | the `claudemaxxing` plugin (skills + hooks) registered from the repo marketplace; agents in `~/.codex/agents`; doctrine pointer in `~/.codex/AGENTS.md` |
...
| tmux | `c` (Claude), `g` (Codex), `o` (OpenCode) session helpers sourced from `~/.config/claudemaxxing/*.sh`, plus a `tmux.conf` snippet included from `~/.tmux.conf` |
...
Codex shows `$claudemaxxing:fanout`, `$claudemaxxing:ideas`, `$claudemaxxing:memory`;
...
`~/.config/claudemaxxing/fleet.env` exists (`KEY=VALUE` per line, `#` comments allowed, no shell
expansion; an explicit environment variable of the same name wins over the file):

```bash
# ~/.config/claudemaxxing/fleet.env — placeholders; every key is optional
CLAUDEMAXXING_SERVER_SSH=you@fleet-server                      # SSH target of the fleet server
CLAUDEMAXXING_SERVER_HOSTNAME=fleet-server                     # its short hostname (sessions there read "local")
CLAUDEMAXXING_DASHBOARD_URL=https://fleet-server.example:5555  # orchestrator dashboard base URL
CLAUDEMAXXING_NOTIFY_TARGET=telegram:<chat>:<thread>           # `hermes send --to` target for done / needs-input
CLAUDEMAXXING_NOTIFY_RELAY=you@fleet-server                    # `tailscale ssh <relay>` when hermes is not local
CLAUDEMAXXING_LAN_PEER=fleet-server.example                    # peer for `harness-sync lan-check`
```

Without the file: the Stop/Notification notifier exits 0 silently, `c` / `g` / `o` skip the
dashboard registration, and `harness-sync lan-check` reports that `CLAUDEMAXXING_LAN_PEER` is not
