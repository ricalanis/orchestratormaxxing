...
Make Codex a first-class host over the existing orchestratormaxxing core. Do not fork the harness into "Claude logic" and "Codex logic." The durable queues, Ollama bridges, session persistence, memory auditor, mutation runner, verifier, and scheduler remain shared. Only host integration lives in adapters.
...
Codex routes nontrivial unplanned work through `$orchestratormaxxing:astraplan` first. Root Codex reviews the Astra Ultra plan and acceptance contract, then invokes `$orchestratormaxxing:fanout` only when the accepted execution shape contains genuinely independent implementation chunks. File count alone never justifies fanout; trivial, already-approved, and tightly coupled execution stays in root Codex.

The `$orchestratormaxxing:fanout` skill therefore operates after required planning and prefers bounded parallel `oll` calls. The optional `ollama-worker` custom agent exists for manual delegation and visible per-agent inspection.
...
The Codex `$orchestratormaxxing:self-improve` contract resolves an ambiguity in the Claude command: it requires exactly two different-family critics for surviving changes.
...
- `.codex/agents/*.toml`: narrow roles (`ollama-worker`, `astra-planner`, `sol-planner`, `product-manager`). `astra-planner` pins `gpt-6-astra` for default planning; the explicit alternative `sol-planner` pins `gpt-5.6-sol` with `model_reasoning_effort = "ultra"` and read-only sandboxing. Delegation stays bounded at depth 1 and four total threads.
- `plugins/orchestratormaxxing/skills/*`: reusable `$orchestratormaxxing:fanout`, `$orchestratormaxxing:ideas`, `$orchestratormaxxing:self-improve`, `$orchestratormaxxing:wrap-up`, `$orchestratormaxxing:astraplan`, `$orchestratormaxxing:solplan`, `$orchestratormaxxing:omaxxing-public-improve`, `$orchestratormaxxing:public-improve-security`, `$orchestratormaxxing:product-manager`, and `$orchestratormaxxing:memory` workflows. Astraplan is the first choice for nontrivial unplanned work and uses GPT-6 Astra Ultra with up to three direct read-only exploration subagents. Its runner closes stdin, suppresses unrelated lifecycle work, captures the final answer, validates the plan contract, and cleans up on cancellation. The Astra planner synthesizes; root Codex reviews the plan and may fan out only independent implementation chunks. Claude's `/fableplan` remains unchanged and Claude-only.
- `plugins/orchestratormaxxing/hooks/hooks.json`: SessionStart/Stop integrations for `mem-audit`, `loop-tick`, and `session-log`.
...
`harness-agent-run` makes the scheduled runner host-neutral, but the default remains Claude. Set `HARNESS_HOST=codex` only after manual `$orchestratormaxxing:self-improve` rounds have produced a green evidence trail. Codex receives an external wall-clock bound; the skill itself bounds provider calls and stops load-bearing changes at `PROPOSED`.

Rollout sequence:

1. Manual `$orchestratormaxxing:fanout`, `$orchestratormaxxing:ideas`, `$orchestratormaxxing:wrap-up`.
2. Manual `$orchestratormaxxing:self-improve` against a synthetic or low-risk queue item.
...
The portable Astra entry point reuses the Sol runner through an explicit profile. Sol remains an explicit alternative. Installed improvement/security skills check complete capability projection, then native cheap-delegate executes bounded contracts; the calling host retains final acceptance.
