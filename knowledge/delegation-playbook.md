...
- **2026-09-06 · Warp parity local measurements · in-session Codex root (GPT-6 family; exact runtime slug unavailable) · INFRA, partial.** A/B retained in root for local environment/desktop observations; C reserved `o / kimi-k2.7-code` but not dispatched because the real quit/relaunch gate was unavailable. Both harness probes returned403 `valid workload token required`; rules CLI returned out-of-credits; desktop unmeasured; AppleScript quit returned user-canceled(-128). Config values on preparation descendants do not prove a running harness session; a blocked generation is not a negative rules/hook tracer. No lane demotion or win. Included/subscription cost unmeasured. Evidence: `.results/delegation/warp-parity-exec-20260906/`.

- **2026-09-06 · config-review · oll / deepseek-v4-flash:0731 · PASS.** One response-only four-item checklist; Root checked JSON/word limits and fresh duplicate discovery, installer fixture, DNS attribution and concurrency-safe rollback requirements. Included quota; dollar cost unmeasured. No lane ranking from one observation. Receipt: `.results/delegation/skills-mcp-cheap-20260906/`.

- **2026-09-06 · warp-guard-parser · o / kimi-k2.7-code · INFRA.** 600-second worker limit; dead pane and two non-returning event-bound handoff calls, both canceled after `o close`. Partial parser changes did not implement deduplication. Root took over with isolated TOML-fragment decoding and governed deduplication/policy; this is not a worker win or evidence to demote its content quality. Receipt: `.results/delegation/warp-parser-20260906/`.

- **2026-09-06 · Astra runner implementation · native Codex / gpt-5.6-sol high · PASS.** Root verified 25 offline lifecycle/subprocess tests including real model argv, read-only isolation and Astra demotion refusal; Sol reused the existing runner. No live model call in tests, dollar cost unmeasured. Receipt: `.results/delegation/astraplan-runner-20260906/`. Independent skill forward review through `oll / glm-5.3` chose correct lanes in four scenarios but FAILED JSON/length contract; retained only as advisory, no Tier-2 repair. Receipt: `.results/delegation/astraplan-forward-20260906/`.

- **2026-09-06 · wide public toolkit · native o / deepseek-v4-flash:0731 + kimi-k2.7-code · mixed, root completion.** Skills and benchmark first attempts failed before provider session binding (infra); retried natively. Skills produced useful drafts but checkpoint semantics required root completion after repair. Examples needed two same-session repairs and root cleanup/interpreter corrections; do not treat asserted cleanup as evidence. Benchmark repair hit its600s runtime deadline without changed files; root implemented nonblocking I/O, group cleanup and telemetry validation. Final root sample18/20 mutation kills; two equivalent/formatting survivors adjudicated, no unresolved behavioral survivors. Final delivered win awaits publication reviews. Gates/receipts: `.results/delegation/weekly-*-20260906*` in isolated public worktrees; private run `.results/weekly-public-contribution/runs/20260906-wide`. Installed older `o handoff` returned typed text but did not persist output.md; root captured exact returned text and recorded the limitation, without changing public o/occ. Included-provider routing; monetary cost unmeasured. No model ranking or win-log entry follows from these attempts.

- **2026-09-05 · Firecrawl WAL acceptance review · oll / glm-5.3 · FAIL (13.65 s, one response).** Conflated separate worker-count and broker experiments and exceeded650word cap. Root retained the valid warning against post-hoc gate replacement, rejected the false adverse corroboration, and kept WAL512 with an inconclusive throughput result. Response-only included-quota review; no repair or new experiment. Receipt: `.results/delegation/firecrawl-wal-gate-review-20260905/receipt.json`.

- **2026-09-05 · Firecrawl stall diagnosis refutation · oll / glm-5.3 · PASS (one bounded response).** Correctly separated Bull worker RAM admission logs from the independent NuQ batch/crawlFinish loops. Root confirmed completed PostgreSQL job rows and a fresh successful batch; earlier no-output inference was a measurement artifact. Added runner progress; restarted baseline. This is diagnosis evidence, not candidate acceptance. Receipt: `.results/delegation/firecrawl-backend-diagnosis-review-20260905/receipt.json`.

- **2026-09-05 · Firecrawl runtime adversarial review · oll / glm-5.3 · format FAIL (13 s, one attempt).** Returned fenced JSON despite a strict JSON-only contract. Root used the bounded findings as advisory evidence: fixed stale version docs and unusable invalid-install remediation; rejected a speculative stderr-consumer regression with no identified consumer. Symlink containment was added and independently tested. Receipt: `.results/delegation/firecrawl-runtime-review-20260905/receipt.json`; no lane ranking follows from this attempt.
...
- **2026-09-05 — service-memory-planning:** response-only `oll`, exact model `deepseek-v4-flash:0731`, one 12.5-second attempt, included subscription (dollar cost not measured). Root prewrote C1-C7; JSON/checklist coverage passed. Critic usefully requested client matrix, drain/restore and CI specificity, but incorrectly claimed several explicit draft requirements were missing; root adjudicated those instead of adopting every finding. Plan-only output; no benchmark/service acceptance. Evidence: `.results/delegation/firecrawl-memory-critic-20260905/`; receipt+ledger bound to output hashes. One advisory observation, not a lane ranking.
...
### 2026-09-05 — altis · POC-1 arnés dbt (bounded-code, OpenCode worker)

| Lane | Chunk | Contract | Result |
|---|---|---|---|
| 2 · `o delegate --profile bounded-code` → `kimi-coder` / `kimi-k2.7-code` | Proyecto dbt-duckdb completo (3 sources CSV externos, 3 staging, schema.yml con contract enforced + 8 tests genéricos, 2 singulares, 2 lotes sintéticos, run.sh, README) | C1–C9 mecánicos: árbol, `run.sh valido` exit 0 / 0 FAIL, `run.sh malformado` exit≠0 con FAIL en not_null+unique+accepted_values+relationships+2 singulares, grep sin credenciales/hosts, contract enforced, datos sintéticos, README ≤ 90 líneas, < 60 s, nada fuera del dir | **PASS 9/9**, intento 1, 0 repairs; verde 29 PASS en 8 s, rojo 8 FAIL en 6 s. Única desviación: `pass:` en vez de `password:` en el target comentado para no ensuciar el grep (aceptada) |

Lección: un brief con la **lista exacta de defectos del lote malformado** y los **pasos de auto-verificación** convierte un scaffold de dbt en tarea de un solo turno para el worker incluido. El worker corrió `dbt build` él mismo antes de entregar; Root repitió ambas corridas desde cero (borrando `target/`) y contó los FAIL por nombre de test — verificación por contrato, no releyendo el código. Sugerencia: para POCs de datos, exigir siempre "verde + rojo" en el contrato; el rojo es lo que demuestra que los tests muerden.
### 2026-09-03 — deck-polish: el bucle multi-lente se estanca en ~95; la última milla la cierra Root midiendo
- Clase: `deck-polish` (llevar un deck de 93-95 a 100 contra rúbrica). Lane: Sonnet vía Workflow para el barrido y los calificadores; **Root ejecuta las correcciones finales**. Verdict: pass en ambas variantes (100/100 diseño y contenido, gate determinista verde).
- Evidencia dura: `04-pia-polish-loop` corrió 4 rondas × 2 variantes (366 agentes, ~35 M tokens) y ninguna cruzó 96. Las notas oscilaban ±4 puntos sobre decks casi idénticos. `05-final-push` (alcance cerrado, 1 lámina por agente + guardia anti-regresión) mejoró contenido de 95 a 98-100 pero dejó diseño en 93-95.
- Lo que sí cerró la brecha: el orquestador midiendo bandas vacías por filas de píxel con Pillow, editando el CSS/HTML puntual, re-renderizando y re-midiendo. Cuatro defectos, cuatro ediciones, dos calificadores nuevos → 100/100. Coste: minutos, no rondas.
- **Dos fallos de diseño del harness, ambos míos:** (1) la rúbrica tenía un tope abierto ("cualquier defecto visible → 96") que hacía el objetivo inalcanzable por construcción; se sustituyó por una lista cerrada de ocho topes más la regla "un defecto resta, una preferencia no". (2) el feed-forward entre rondas llevaba solo `to_reach_97` y no `blocking`, así que un tope duro sobrevivió cuatro rondas sin que nadie lo atacara.
- Regla derivada: **un umbral solo es alcanzable si el criterio para no alcanzarlo es enumerable y verificable.** Si un calificador puede inventar el defecto, el bucle gasta cómputo en ruido de calificación, no en calidad. Y por encima de ~95 los agentes introducen tantas regresiones como mejoras: cerrar a mano lo geométrico.

### 2026-09-03 — deck-generation (PIA Sofía): lane Sonnet-Workflow, 4 workflows, ambas variantes entregadas
- Clase: `deck-generation` (presentación académica desde un documento fuente). Lane: Claude Sonnet vía Workflow (generación + jueces + críticos); Fable solo orquestó, verificó por contrato y cerró exports. Verdict: pass (2 decks de 16 láminas, PDFs, notas, procedencia 0 fugas). Modelo: `claude-sonnet-5` (slug medido en el error 429 del harness).
- Ollama one-shot descartado en spec-gate: cada subtarea exigía web (verificar cifras/APA) o visión (juez de PNG), que `oll` no tiene. Codex no usado.
- Sorpresas: (a) límite de sesión Sonnet (reset 02:30) tumbó los últimos agentes de un workflow de 82 → los pasos mecánicos (export/README) deben ser scripts, no agentes; (b) el gate determinista de procedencia en Python (números del HTML ∈ texto fuente ∪ [calc]) rindió más que cualquier crítico LLM; (c) un juez visual por lámina con Read de PNG aprobó 29/32 láminas a la primera; (d) bucle crítica→revisión debe cerrar con verificación, no con revisión.
- Costo: ~24 M tokens de subagentes (subscription lane), ~7 h de reloj con 3 workflows en paralelo. Metodología reutilizable: `~/dev/sofia/docs/metodologia-presentacion.md` + `docs/workflows/*.js`.
...
- **2026-09-02 · oll (deepseek-v4-flash:0731) · docs-draft · pass (attempt 2; attempt 1 = infra DNS failure, retried same lane).** 6 personas + 6 journeys en español desde un brief de ~2,800 palabras (digest del Atlas + reunión); contrato de 8 checks grep-eables pasó 12/12 sin repair. Sorpresa: mi verify.sh tenía un falso negativo (el preámbulo contaba como journey) y disparó una repair round innecesaria — validar el validador contra un caso conocido antes de dispatch. Calidad: buena estructura, nombres ficticios plausibles; el crítico Claude sigue siendo necesario para C8 (cifras no verificables por grep).
- **2026-09-02 · oll (deepseek-v4-flash:0731) · data-extract · pass (attempt 1, 0 repairs, 26/26).** Extracción PDF-texto-con-columnas-mezcladas → JSON estricto por ficha, 4 en paralelo, temp 0.1, ~13.9k palabras de fuente en 26 llamadas. Contrato fuerte y barato: "todo número de 2+ dígitos del JSON debe existir en la fuente" (regex) + claves exactas + título igual al índice. Cero cifras inventadas en 26 fichas. Lección operativa (harness, no modelo): `pkill -f` con un patrón que también aparece en el propio comando mata el shell de la herramienta (exit 144) — usar patrones indirectos (`'extrac' + 't.sh'`) o PIDs; y el cwd del Bash tool persiste entre llamadas paralelas: usar rutas absolutas siempre en pipelines de delegación.
- **2026-09-02 · oll --reasoning (glm-5.3) · ui-fragment · pass (attempt 1, 1 repair round en F2/F3).** Tres fragmentos HTML+CSS de un documento editorial (identidad/personas/journeys/mapa/preguntas) desde briefs de 2.5k–7.4k palabras con tokens y clases compartidas dados. Contrato D1–D6 grep-eable (ids exactos, hex ⊂ tokens, sin sombras/gradientes/pills/emoji, 8 nombres, 12 ids de sección, ≥15 preguntas). Fallas de ronda 1: un hex (#5675ed del spec vs #5775ed de tokens) y una mención institucional — ambas reparadas con diff de falla. Calidad visual: buena jerarquía editorial, pero abusa de `.display` a tamaño gigante en cada h2/h3; Root corrigió con un override CSS de 4 líneas en el ensamblador. Latencia ~6–9 min por fragmento. Lección: cuando el lane barato hace layout, meter en el brief una escala tipográfica por nivel explícita, no solo tokens.
- **2026-09-02 · codex Luna (gpt-5.6-luna, read-only) · ui-audit · pass (attempt 1).** Auditoría de accesibilidad/teclado/responsive/no-JS sobre un index.html de 250 KB + spec + partes: JSON válido, 8 hallazgos con línea exacta, 7 P1 + 1 P2; 5 aceptados (anclas sin JS, Home/End en Ampliar, copy de persistencia, role=listitem en botones, título estático), 3 rechazados con razón (contenido del propio Atlas, alcance de prototipo, decisión de breakpoint defendible). ~20 min de pared. Sustituyó a tres subagentes Claude que murieron por límite de sesión: para auditorías de código con spec escrita, Luna read-only es el lane por defecto.

- **2026-09-02 · altis · lane `oll` (`deepseek-v4-flash:0731`) · clases `structured-doc-draft-es`, `client-doc-draft-es`, `table-doc-derivation-es`.** Tres borradores cara al cliente. Los tres **pasaron el contrato mecánico** (estructura, literales, strings prohibidos, tabla celda por celda) en ≤10 s y con 0–1 ronda de reparación, y los tres **fallaron el gate adversarial** de segunda capa (9 refutadores Fable, lentes fidelidad/tono/fuga: 6 BLOCKER · 45 MAJOR). Patrón único: el modelo escribe para un ingeniero (jerga de pipeline, códigos B1–B4/H1–H7 sin definir, confirmaciones de arquitectura pre-contestadas) y **fabrica reglas plausibles** que no están en la fuente (Fan Score "base 100", estados de campaña, canales de consentimiento). El lane sí es adecuado para la guía interna y para derivaciones celda por celda. **Regla nueva:** un documento que un cliente no técnico va a reenviar sin editar no es delegable a `oll` aunque el contrato mecánico sea enunciable — el contrato no puede capturar "legible por Rubén"; se queda en la sesión frontera y `oll` se usa, si acaso, para la guía interna. Sorpresa: la reparación mecánica converge en una ronda, pero la reparación de tono/fidelidad no es especificable como diff → techo de especificación, no de capacidad del lane.
- **2026-09-02 · oll --reasoning (glm-5.3) · ui-fragment (refinamiento) · pass (attempt 1, 0 repairs; 12.9k tokens de salida).** Brief de 4.5k palabras con lista explícita de 11 bloques, assets con descripción y clases exactas; el contrato grep-eable (imgs válidas, tokens de escala, clases de retícula, encabezados de componentes, nowrap) pasó a la primera. **Falla no cubierta por el contrato:** `.dz-id-bloque{height:12rem}` en el wrapper de cada bloque → toda la sección encimada (3,190 px de alto); y un grid con texto `nowrap` a 20rem reventó la tercera columna (grid blowout, falta `min-width:0`). Regla nueva para fragmentos de layout: el contrato debe incluir (a) prohibición de height/block-size fijo fuera de demos y (b) un check de render determinista (altura de la sección a 1280 ≥ N px con Chrome headless + PIL) antes de aceptar. Root cerró con 1 regla + 2 overrides de CSS.

- **2026-09-05 · altis · lane `oll` (`deepseek-v4-flash:0731`) · clase `structured-json-derivation-es`.** Derivación de 11 objetos JSON (semanas S03–S13 de un sitio interno de preparación) desde tablas y secciones de un plan, con esquema fijo, fechas y celdas verbatim, rangos de ítems por campo y strings prohibidos. **Pass** en 1 intento (5.9k in / 4.4k out, ~10 s): 11/11 objetos válidos, fechas/hito/meta/h idénticos a la fuente, 0 prohibidos; solo 2 títulos traían códigos (`H2`, `H3`) pese a la regla — root los corrigió en una línea en vez de una ronda de reparación. Confirma la regla de la entrada del 09-02: `oll` es adecuado para **derivaciones estructuradas con contrato mecánico y audiencia interna**; el fallo residual es siempre el mismo (respeta la estructura, se le escapan restricciones de vocabulario en campos cortos).
- **2026-09-05 · o delegate --profile reasoning (glm-5.3) · ui-fix · infra (turn timeout).** Un brief de 24 correcciones sobre 9 archivos (fanzine.js de 56 KB incluido) murió a los 600 s por defecto (`O_TURN_TIMEOUT_SECONDS`) sin escribir nada: "Pane is dead (status 1)". Regla nueva: para ui-fix con >8 cambios o archivos >30 KB, (a) exportar `O_TURN_TIMEOUT_SECONDS=1800` y `O_HANDOFF_TIMEOUT_SECONDS=2400` antes de `o delegate`, y (b) partir por archivo en lotes disjuntos que corran en paralelo (cada uno con su ronda de verify.sh para no pisar evidencia). Codex `gpt-5.6-astra` no existe en este harness (auth/timeout); los slugs vivos siguen siendo luna/terra/sol.
- **2026-09-05 · o delegate --profile reasoning (glm-5.3) · ui-fix sobre archivos grandes · fail ×3 (finish=length).** Tres intentos (11, 7 y 4 correcciones) sobre fanzine.css (31 KB) y fanzine.js (56 KB) terminaron en `incomplete_tool_call` por longitud de salida: el agente intenta reescribir el archivo completo en una llamada de herramienta. Regla: para archivos >25 KB, o se instruye explícitamente "usa edit con search/replace mínimo, nunca reescribas el archivo" y se limita a ≤4 cambios por turno, o Root aplica los cambios (spec ceiling). Los lotes pequeños sobre archivos <10 KB (indice, sobre, participa, cta, base, landing-top) sí pasaron a la primera con verify.sh en verde.
- **2026-09-05 · o delegate --profile planning (kimi-k3) · ui-review de 96 ítems · fail (límite de pasos de la sesión).** La ronda 0 cupo en un turno; la ronda 1, con más archivos que leer, agotó el presupuesto de pasos del worker antes del handoff JSON. Regla: revisiones grandes se parten en sesiones paralelas por grupos de criterios (≈30 ítems cada una) con un digest de evidencia precomputado (probe/metrics/diff de la ronda), y el grader fusiona; nunca una sola sesión con >40 ítems y >10 archivos.

- **2026-09-06 · weekly-public-contribution · root Codex review closure.** GLM5.3 public-text review renewal, Kimi-k2.7-code immutable-archive static scan, and fresh Qwen3.5:397b independent review completed with canonical receipts. Reviewer observations required root counterexamples and scope adjudication; several unsupported-input claims were false positives. Live Docker source binds twice disagreed with clean Git files; immutable archive removed that ambiguity. Runner/scorer mutation each18/20: threshold pass with survivors, no win-log. Full native GLM review timeout and format-budget retries remain recorded, not erased by final success. PR14 published via existing SSH after OAuth lacked workflow scope.
- **2026-09-05 · o delegate --profile planning (kimi-k3) · ui-review de 96 ítems en 3 grupos · pass con ruido (13/24 FAIL «no verificable»).** Sin ojos y sin presupuesto para abrir código, el revisor marca FAIL lo que no puede ver (capturas) o no leyó (K). Arreglo de contrato para la ronda siguiente: el digest lleva `facts.json` (hechos de código deterministas por ítem, archivo:línea) y `root-visual.json` (veredictos del orquestador con visión, una observación por captura), y la regla «fail solo con evidencia concreta en contra; nunca por no verificable». La calificación determinista (grade.py) cubre 94/96 antes de que el revisor entre; el revisor solo aporta juicio, no cobertura.
